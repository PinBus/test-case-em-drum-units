...
// Starts out on false, as it has to wait for the initial reset and it is false during regular resets
bool state_is_drum_unit_allowed = false;

// Is false when the reset button can not be used
bool state_is_reset_allowed = false;

// Timer to keep track of the cooldown on buttons
uint32_t state_last_button_press = 0;
...
void start_button_cooldown() {
    if (state_last_button_press != 0) {
        return;
    }

    state_last_button_press = millis();

    visualize_buttons();
}

void update_button_cooldown() {
    if (state_last_button_press == 0) {
        // No active cooldown
        return;
    }

    if (drum_unit_reset_timer != 0) {
        // Do not enable the buttons as long as the reset sequence is running.
        return;
    }

    if (millis() - state_last_button_press > BUTTON_COOLDOWN_MS) {
        state_last_button_press = 0;

        visualize_buttons();
    }
}

bool is_drum_button_allowed() {
    return state_last_button_press == 0 && state_is_drum_unit_allowed;
}

bool is_reset_button_allowed() {
    return state_last_button_press == 0 && state_is_reset_allowed;
}

void visualize_buttons() {
    uint16_t reset_button_value = is_reset_button_allowed() ? LED_MAX_VALUE : LED_QUARTER_VALUE;
    uint16_t drum_button_value = is_drum_button_allowed() ? LED_MAX_VALUE : LED_QUARTER_VALUE;

    pca9685.setPin(PIN_PCA9685_LED_BUTTON_RESET, reset_button_value, true);
    pca9685.setPin(PIN_PCA9685_DRUM_UNIT_1, drum_button_value, true);
    pca9685.setPin(PIN_PCA9685_DRUM_UNIT_10, drum_button_value, true);
    pca9685.setPin(PIN_PCA9685_DRUM_UNIT_100, drum_button_value, true);
    pca9685.setPin(PIN_PCA9685_DRUM_UNIT_1000, drum_button_value, true);
}
...
void on_1000_button_down() {
    if (!is_drum_button_allowed() || drum_units[0].timer) {
...
void on_100_button_down() {
    if (!is_drum_button_allowed() || drum_units[1].timer) {
        // Prevent firing drum unit while resetting or while firing
        return;
    }
...
void on_10_button_down() {
    if (!is_drum_button_allowed() || drum_units[2].timer) {
        // Prevent firing drum unit while resetting or while firing
        return;
    }
...
void on_1_button_down() {
    if (!is_drum_button_allowed() || drum_units[3].timer) {
        // Prevent firing drum unit while resetting or while firing
        return;
    }
...
void setup() {
    ...
    // Visualize all buttons in a disabled state, until the initial reset has happened
    visualize_buttons();
}
